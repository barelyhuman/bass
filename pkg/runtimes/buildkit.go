...
func (b *buildkitBuilder) image(ctx context.Context, image *bass.ThunkImage) (llb.State, llb.State, string, bool, error) {
	...
	if image.Archive != nil {
		...
		if err != nil {
			return llb.State{}, llb.State{}, "", false, fmt.Errorf("image archive resolve index: %w", err)
		}

		manifestBlob, err := content.ReadBlob(ctx, b.runtime.ociStore, *manifestDesc)
		if err != nil {
			return llb.State{}, llb.State{}, "", false, fmt.Errorf("image archive read manifest blob: %w", err)
		}
