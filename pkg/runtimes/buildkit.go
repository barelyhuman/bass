...
func (runtime *Buildkit) Resolve(ctx context.Context, imageRef bass.ImageRef) (bass.ImageRef, error) {
	...
	if err != nil {
		return bass.ImageRef{}, fmt.Errorf("resolve image config: %w", err)
	}
...
func (b *buildkitBuilder) Start(ctx context.Context, thunk bass.Thunk) (StartResult, error) {
	...
	ib, err := b.Build(ctx, thunk, getWorkdir)
	if err != nil {
...
func (runtime *Buildkit) build(
	...
	err := runtime.WithGateway(ctx, func(ctx context.Context, gw gwclient.Client) (*gwclient.Result, error) {
		...
		ib, err := b.Build(ctx, thunk, transform, runOpts...)
		if err != nil {
			return nil, err
		}
...
func (b *buildkitBuilder) image(ctx context.Context, image *bass.ThunkImage) (ib IntermediateBuild, err error) {
	...
	case image.Thunk != nil:
		return b.Build(ctx, *image.Thunk, getWorkdir)
	...
	case image.Archive != nil:
		...
		indexBlob, err := content.ReadBlob(ctx, b.ociStore, desc)
		if err != nil {
		...
		platform := platforms.Only(b.platform)
		...
		for _, m := range idx.Manifests {
			...
			manifestBlob, err := content.ReadBlob(ctx, b.ociStore, m)
			if err != nil {
				return ib, fmt.Errorf("image archive read blob: %w", err)
			}
			...
			configBlob, err := content.ReadBlob(ctx, b.ociStore, man.Config)
			if err != nil {
				return ib, fmt.Errorf("image archive read blob: %w", err)
			}
		...
		return ib, fmt.Errorf("image archive had no matching manifest for platform %s and tag %s", platforms.Format(b.platform), tag)
	}
...
func (b *buildkitBuilder) thunkPathSt(ctx context.Context, source bass.ThunkPath) (llb.State, string, bool, error) {
	ib, err := b.Build(ctx, source.Thunk, getWorkdir)
	if err != nil {
